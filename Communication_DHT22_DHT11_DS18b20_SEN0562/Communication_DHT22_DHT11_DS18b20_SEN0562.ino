...
#include <BH1750.h>          // Capteur de luminosité BH1750 (SEN0562)
...
float tempDS18B20;
float luminosite;
byte buffer[12]; // Stocke les valeurs à envoyer (2 octets * 6 valeurs)
...
void setup() {
  ...
  // Initialisation du capteur BH1750 (luminosité)
  Wire.begin();
  if (!lightMeter.begin()) {
...
///// MESURE DE LUMINOSITÉ - BH1750 /////
void measureLuminosity() {
  Serial.println("Lecture du capteur de luminosité BH1750...");
  luminosite = lightMeter.readLightLevel(); // Lecture en lux

  Serial.print("Luminosité : ");
  Serial.print(luminosite);
  Serial.println(" Lux");

  buildMessage(luminosite, 10); // Stocke la valeur dans le buffer
}
...
void sendMessage() {
  ...
  modem.beginPacket();
  modem.write(buffer, 12); // 12 octets (2 octets * 6 valeurs)
